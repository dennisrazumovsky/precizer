...
* test database by path before open or compare

* Progress bar with escape
...
* Add DB checks before any changes
	How should I use Sqlite "PRAGMA integrity_check" in C
	https://stackoverflow.com/questions/22172186/how-should-i-use-sqlite-pragma-integrity-check-in-c

* --ignore=PATH(or PCRE regexp) and --include=PATH(or PCRE regexp) (Consider the PATH even it is a subfolder of a file under --ignore directory)
* --maxdepth option (look at fts_level https://man7.org/linux/man-pages/man3/fts.3.html)
* --no-recursive option
...
* Default file name of the database date_startpath_hostname.db
* ALL DONE
